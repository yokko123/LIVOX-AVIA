...
class ROS_MODULE:
    ...
    def ros_print(self, pts, pred_dicts=None, last_box_num=None, gt_boxes=None, last_gtbox_num=None):
        ...
        def calculate_box_distance(boxes):
           ...
           for i in range(num_boxes):
                ...
                # Calculate the distance from the origin to the center point
        ...
        if pred_dicts is not None:
            ...
            dist = calculate_box_distance(boxes)
            # print(boxes)
            ...
            for obid in range(boxes.shape[0]):
                ...
                markert_distance = Marker()
                markert_distance.header.frame_id = header.frame_id
                markert_distance.header.stamp = header.stamp
                markert_distance.id = obid * 2 + 2  # Increment the ID
                markert_distance.action = Marker.ADD
                markert_distance.type = Marker.TEXT_VIEW_FACING
                markert_distance.lifetime = rospy.Duration(0)

                # Set the position and text for distance
                markert_distance.pose.orientation.w = 1.0
                markert_distance.pose.position.x = (ob[0][0] + ob[2][0]) / 2
                markert_distance.pose.position.y = (ob[0][1] + ob[2][1]) / 2
                markert_distance.pose.position.z = (ob[0][2] + ob[4][2]) / 2 + 1.5  # Adjust the z-position as needed
                markert_distance.text = "{:.2f}".format(distance)+"m"
                # print(distance)
                markert_distance.scale.x = 1.0  # Set the scale
                markert_distance.scale.y = 1.0
                markert_distance.scale.z = 0.6

                markert_distance.color.a = 1.0  # Set the color (white)
                markert_distance.color.r = 1.0
                markert_distance.color.g = 1.0
                markert_distance.color.b = 1.0
                marker_array_text.markers.append(markert_distance)
            # clear ros cache   
