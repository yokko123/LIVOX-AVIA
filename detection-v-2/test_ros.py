...
def boxes_to_corners_3d(boxes3d):
    """
    Args:https://levelup.gitconnected.com/fix-attributeerror-module-numpy-has-no-attribute-float-d7d68c5a4971
...
class ros_demo():
    ...
    def receive_from_ros(self, msg):
        ...
        points_list[:, 0] = copy.deepcopy(np.float(pc['x']))
        points_list[:, 1] = copy.deepcopy(np.float(pc['y']))
        points_list[:, 2] = copy.deepcopy(np.float(pc['z']))
        # points_list[:, 3] = copy.deepcopy(np.float(pc['intensity']))
